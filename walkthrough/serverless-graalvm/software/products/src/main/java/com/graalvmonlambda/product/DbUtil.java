...
/**
 * Database Utility class. Used for connecting to database to execute RDS commands.
 * Its provide two authentication method. One is with DB username and password.
 * Another is via IAM Auth. In the IAM Auth method, the client doesn't need to know the password
 * but it needs IAM permission to use that username. RDS Proxy will fetch password from
 * a Secret Store.
 */
public class DbUtil {
  private static final String SSL_CERTIFICATE = "rds-ca-2019-eu-west-2.pem";
 ...
 // private static final Logger logger = LogManager.getLogger(CreateSlots.class);
  ...
  /**
   * Create a database connection via a IAM Authentication.
